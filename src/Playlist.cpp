...
m3uPlaylist::m3uPlaylist(const std::string &_Name, const std::string &_path)
    : Playlist(_Name), path{_path} {
    if (!(std::filesystem::exists(this->path) &&
          std::filesystem::is_directory(this->path))) {
...
void m3uPlaylist::generate() {
    ...
    if (!fichier) {
        throw std::ios::failure("Impossible de créer le fichier : " +
                                this->getFileName());
    }
